...
const CURRENT_STORAGE_VERSION = '2026-06-12-deepseek-text-only'
...
export function HomePage() {
  ...
  async function postChatMessage(activeSessionId: string, content: string) {
    return apiPost<ChatMessagePairResponse>(`/api/chat/sessions/${activeSessionId}/messages`, {
      content,
    })
  }
  ...
  async function sendMessage(content: string) {
    ...
    try {
      ...
      try {
        response = await postChatMessage(activeSessionId, trimmedContent)
      } catch (error) {
        if (!(error instanceof ApiError) || error.status !== 404) throw error

        localStorage.removeItem(STORAGE_SESSION_ID)
        const newSessionId = await createSession()
        response = await postChatMessage(newSessionId, trimmedContent)
      }
